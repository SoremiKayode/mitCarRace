...
5. Use `TurnLeft` and `TurnRight` to move only across the x-axis while the car stays facing forward; use `Accelerate` and `Brake` from your own buttons or a clock if needed.
...
The extension now requests landscape mode by default, draws built-in left and right navigation buttons on the game surface, and includes polished icon-only Start, Stop, Pause, Play, Scores, and transparent close-sidebar controls with touch wave feedback. Start is hidden after the game starts, Stop ends the current run without saving a finish score or leaving stale pressed controls, Pause stops the game loop, and Play resumes without resetting race state.

New beginner blocks include configurable road builders, accelerometer steering, local score storage/retrieval, high-score retrieval, open/close score sidebar controls, numbered opponent image directions, and a professional in-game notifier banner. See [`docs/BlocksGuide.md`](docs/BlocksGuide.md) for a full block-by-block tutorial and recommended setup workflow.
